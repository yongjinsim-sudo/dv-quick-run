...
export type DataverseListResponse<T> = { value?: T[] };

export type CustomApiRecord = Record<string, unknown> & {
  customapiid?: string;
  uniquename?: string;
  name?: string;
  displayname?: string;
...
export type CustomApiRequestParameterRecord = Record<string, unknown> & {
...
export type CustomApiResponsePropertyRecord = Record<string, unknown> & {
...
export const CUSTOM_API_QUERY = "/customapis?$select=customapiid,uniquename,name,displayname,description,isfunction,bindingtype,boundentitylogicalname,executeprivilegename,allowedcustomprocessingsteptype,isprivate&$orderby=uniquename asc";
export const CUSTOM_API_REQUEST_PARAMETER_QUERY = "/customapirequestparameters?$select=customapirequestparameterid,uniquename,name,displayname,type,isoptional,logicalentityname,_customapiid_value&$orderby=uniquename asc";
export const CUSTOM_API_RESPONSE_PROPERTY_QUERY = "/customapiresponseproperties?$select=customapiresponsepropertyid,uniquename,name,displayname,type,_customapiid_value&$orderby=uniquename asc";
...
function normalizeBindingKind(record: CustomApiRecord): CustomApiBindingKind {
  if (toText(record.boundentitylogicalname)) return "Bound";
  if (record.bindingtype === 0) return "Unbound";
  if (typeof record.bindingtype === "number") return record.bindingtype > 0 ? "Bound" : "Unbound";
  return "Unknown";
}
function normalizeBoundTargetKind(record: CustomApiRecord): CustomApiBoundTargetKind {
  if (record.bindingtype === 0) return "none";
  if (record.bindingtype === 1) return "entity";
  if (record.bindingtype === 2) return "collection";
  if (toText(record.boundentitylogicalname)) return "entity";
  if (typeof record.bindingtype === "number" && record.bindingtype > 0) return "unknown";
  return normalizeBindingKind(record) === "Bound" ? "unknown" : "none";
}
function describeBoundTargetKind(targetKind: CustomApiBoundTargetKind, boundEntityLogicalName: string | undefined): Pick<CustomApiDefinition, "boundTargetLabel" | "boundTargetReason"> {
  if (targetKind === "entity") return {
  ...
  if (targetKind === "collection") return {
  ...
  return { boundTargetLabel: "Unbound", boundTargetReason: "This operation is not bound to a specific Dataverse row or collection." };
}
...
function mapRequestParameter(record: CustomApiRequestParameterRecord): CustomApiRequestParameter {
  ...
  const metadata = getCustomApiTypeMetadata(type);
  return {
    ...
    typeCategory: metadata.category,
    typeDescription: metadata.description,
    executionSupport: metadata.executionSupport,
    executionSupportLabel: metadata.supportLabel,
    executionSupportReason: metadata.supportReason,
...
function groupByCustomApiId<T extends { _customapiid_value?: string }>(items: readonly T[]): Map<string, T[]> {
  const grouped = new Map<string, T[]>();
  for (const item of items) {
    const id = toText(item._customapiid_value);
    if (!id) continue;
...
function buildExecutionReadiness(parameters: CustomApiRequestParameter[]): Pick<CustomApiDefinition, "previewReadyParameterCount" | "inspectOnlyParameterCount" | "executionReadiness" | "executionReadinessLabel" | "executionReadinessReason"> {
  const inspectOnlyParameterCount = parameters.filter((parameter) => parameter.executionSupport !== "preview-ready").length;
  const previewReadyParameterCount = parameters.length - inspectOnlyParameterCount;
  if (parameters.length === 0 || inspectOnlyParameterCount === 0) return {
    previewReadyParameterCount, inspectOnlyParameterCount, executionReadiness: "preview-ready", executionReadinessLabel: "Preview-ready",
    executionReadinessReason: "All discovered parameters use simple types supported by the preview foundation."
  };
  if (previewReadyParameterCount > 0) return {
    previewReadyParameterCount, inspectOnlyParameterCount, executionReadiness: "partial", executionReadinessLabel: "Partially preview-ready",
    executionReadinessReason: "Some parameters use complex types that need additional payload support before execution preview."
  };
  return {
    previewReadyParameterCount, inspectOnlyParameterCount, executionReadiness: "inspect-only", executionReadinessLabel: "Inspect only",
...
function mapDefinition(record: CustomApiRecord, requestParameters: CustomApiRequestParameterRecord[], responseProperties: CustomApiResponsePropertyRecord[]): CustomApiDefinition {
  ...
  return {
    ...
    responseProperties: responseProperties.map(mapResponseProperty),
    ...buildExecutionReadiness(mappedRequestParameters)
  };
}

export function mapCustomApiDefinitions(
  apiRecords: readonly CustomApiRecord[],
  requestParameterRecords: readonly CustomApiRequestParameterRecord[],
  responsePropertyRecords: readonly CustomApiResponsePropertyRecord[]
): CustomApiDefinition[] {
  const requestByApi = groupByCustomApiId(requestParameterRecords);
  const responseByApi = groupByCustomApiId(responsePropertyRecords);
  return apiRecords.map((record) => {
    const id = toText(record.customapiid) ?? "";
    return mapDefinition(record, requestByApi.get(id) ?? [], responseByApi.get(id) ?? []);
  });
}

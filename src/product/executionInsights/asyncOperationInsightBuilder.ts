...
function buildConsolidatedAsyncOperationInsight(group: AsyncOperationGroup): BinderSuggestion {
  ...
  const isPrimarySignal = isPrimaryAsyncOperationSignal(group, repeatPattern);
  const summary = buildPrimarySignalSummary(group, repeatPattern);
  const guidedInvestigationSteps = buildGuidedInvestigationSteps(group, repeatPattern);
  const relatedSignals = isPrimarySignal ? buildRelatedSignals(repeatPattern) : [];

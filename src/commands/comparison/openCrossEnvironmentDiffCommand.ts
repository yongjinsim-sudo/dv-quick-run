...
interface InvestigationSessionState {
...
function getComparisonEntityLogicalName(snapshots: readonly ComparisonSnapshotFile[]): string | undefined {
...
function buildDefaultExportUri(model: ComparisonViewModel, extension: "json" | "md" | "html" | "baseline"): vscode.Uri | undefined {
...
async function saveComparisonExport(model: ComparisonViewModel, kind: "json" | "md" | "html" | "baseline"): Promise<vscode.Uri | undefined> {
  if (!canRunCrossEnvironmentDiff()) {
  ...
  const uri = await vscode.window.showSaveDialog({
    ...
    filters,
    saveLabel: kind === "json" ? "Save JSON" : kind === "md" ? "Save MD" : kind === "baseline" ? "Export Baseline Diff" : "Save HTML",
    title: kind === "json"
      ? `Save ${exportTitle} JSON`
      : kind === "md"
        ? `Save ${exportTitle} Markdown`
        : kind === "baseline"
          ? `Export Pre-investigation Baseline Diff`
          : `Save ${exportTitle} HTML`
  });
  ...
  void vscode.window.showInformationMessage(`DV Quick Run: Saved ${kind === "baseline" ? "Pre-investigation Baseline Diff" : model.title.startsWith("Timeline Diff") ? "Timeline Diff" : "Cross-Environment Diff"} ${kind === "baseline" ? "HTML" : kind.toUpperCase()} to ${uri.fsPath}.`);
  return uri;
}
...
function buildInvestigationSessionKey(model: ComparisonViewModel): string {
...
function readInvestigationSessionState(
  context: vscode.ExtensionContext,
...
async function clearInvestigationSessionState(
  context: vscode.ExtensionContext,
  comparisonKey: string
): Promise<void> {
  const entries = context.globalState.get<readonly InvestigationSessionStateEntry[]>(INVESTIGATION_SESSION_STATE_KEY, []);
...
async function writeInvestigationSessionState(
  context: vscode.ExtensionContext,
  comparisonKey: string,
...
function revealComparisonSurface(ctx: CommandContext, model: ComparisonViewModel): void {
  ...
  comparisonPanelMessageDisposable = comparisonPanel.webview.onDidReceiveMessage((message: unknown) => {
    ...
    if (request.kind === "json" || request.kind === "md" || request.kind === "html" || request.kind === "baseline") {
      void saveComparisonExport(model, request.kind).then((savedUri) => {
...
async function buildEvidencePivotResult(
  ctx: CommandContext,
  evidenceKind: string | undefined,
  label: string | undefined,
  value: string | undefined,
  ...
  if (metadataPivotResult) {
    appendEvidencePivotDiagnostic(ctx, "route.metadataContext", { label, value, parentTitle, entityLogicalName: comparisonEntityLogicalName });
    return metadataPivotResult;
  }

  if (kind.includes("solution") || text.includes("solution") || parentText.includes("solution")) {
    appendEvidencePivotDiagnostic(ctx, "route.solution", { label, value, parentTitle, entityLogicalName: comparisonEntityLogicalName });
  ...
  if (customEntityLogicalNames.length > 0) {
    appendEvidencePivotDiagnostic(ctx, "route.entityMetadata", { label, value, parentTitle, entityLogicalName: comparisonEntityLogicalName, customEntityLogicalNames });

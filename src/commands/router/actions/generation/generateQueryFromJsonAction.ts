...
import { ParsedRecord } from "./shared/generateQueryFromJsonTypes.js";
import {
  readJsonFromEditor,
  tryParseDataverseRecord,
  tryParseEntitySetFromContext
} from "./shared/generateQueryFromJsonParser.js";
...
import {
  ...
  buildFullUrl,
  buildCurlGet
} from "./shared/generateQueryFromJsonQueryBuilder.js";
...
export async function runGenerateQueryFromJsonAction(ctx: CommandContext): Promise<void> {
  await runAction(ctx, "DV Quick Run: Generate Query From JSON failed. Check Output.", async () => {
    ...
    logInfo(ctx.output, `Generate Query From JSON: used=${payload.used} source=${rec.source} entity=${rec.entitySetName}`);
    logDebug(ctx.output, `Record id: ${rec.id}`);
    if (rec.primaryIdField) {
      logDebug(ctx.output, `PrimaryIdField: ${rec.primaryIdField}`);
    }
    logDebug(ctx.output, `Direct GET: /${buildGetPath(rec)}`);
    logDebug(ctx.output, `Direct GET + $select: /${buildGetPathWithSelect(rec)}`);
    logDebug(ctx.output, `Filter by ID: /${buildFilterPathById(rec)}`);

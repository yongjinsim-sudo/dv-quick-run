...
suite("miniRcaEngine", () => {
  ...
  test("renders versioned Evidence Relationships and two-column Mini RCA report sections", () => {
    ...
    const releaseVersion = getDvqrReleaseVersion();
    assert.ok(markdown.includes(`Generated by: DV Quick Run v${releaseVersion}`));
    assert.ok(html.includes(`DV Quick Run v${releaseVersion}`));
    assert.doesNotMatch(markdown, /Generated by: DV Quick Run v0\.15\.3/);

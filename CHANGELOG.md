...
All notable changes to **DV Quick Run** are documented in this file.
...
- Added a layered **Business Capability Landscape** covering Core Domain, Coordination, Execution, Governance and Platform.
- Added multidimensional capability profiles for Governance, Domain, Scheduling, Coordination, Execution and Integration.
- Added source-first Operational Anchor Discovery so callers can begin from a business record or table without nominating a Task table.
- Added explainable architectural conclusion cards for the primary business capability, operational anchor, governance layer, scheduling layer, integration layer and work-execution layer.
- Added confidence, evidence provenance, selection reasons, runtime status and uncertainty to major architectural conclusions.
- Added a plain-English architectural narrative and onboarding-oriented **If I joined tomorrow** summary.
- Added bounded workflow-family discovery beyond direct relationships so materially different bridged workflows remain visible.
- Added investigation-scoped runtime re-ranking while preserving structural metadata as the primary recommendation basis.
- Added deterministic target-concept expansion for generic targets such as `task`, allowing related custom task tables to be evaluated without treating them as the same schema entity.
- Added separate `metadataRecommendation` and `runtimeRecommendation` outputs.
- Added candidate-family diversity so near-identical direct relationship roles do not consume the complete inline result.
- Added a portable text mirror for structured MCP tool results in Visual Studio and other text-only MCP hosts while preserving `structuredContent` for richer clients.
- Added bounded text-mirror truncation with an explicit portable-envelope notice.
- Added `DVQR_MCP_EMIT_TEXT_MIRROR` and `DVQR_MCP_TEXT_MIRROR_MAX_CHARACTERS` configuration.

### Improved

- Operational anchors, supporting anchors and downstream work items are now communicated separately.
- Confidence and uncertainty are shown inline instead of being deferred to a final limitations section.
- Runtime results distinguish observed data, no matching data, permission-limited evidence and exhausted probe budgets.
- Relationship discovery now probes diverse relationship families within a shared GET budget.
- Valid record-navigation paths generated by relationship intelligence, including paths such as `contacts(<guid>)/Contact_Tasks`, now execute correctly through MCP OData execution.
- Metadata confidence is separated from business interpretation so deterministic traversal ranking is not presented as organisational preference.
- Relationship guidance uses explicit metadata-ranked traversal semantics and an `UnknownFromMetadata` business-confidence boundary.
- OData execution summaries now include returned-record counts and truncation state.
- Corporate TLS inspection guidance now covers `SELF_SIGNED_CERT_IN_CHAIN` and `NODE_EXTRA_CA_CERTS`; the bounded Windows PowerShell fallback remains available.
- Refreshed the Welcome page around **Understand Business Architecture**.
- Refreshed the Hub with capability-oriented prompts, Local MCP status, operational-anchor guidance and v0.15.6 release highlights.
- Consolidated README messaging around Local MCP, Business Architecture Understanding and the editor investigation workflow.
- Updated Marketplace metadata to lead with Local MCP, natural-language Dataverse investigation and business architecture understanding.

### Evidence and safety boundaries

- Capability classification is metadata-derived and does not prove runtime participation.
- Work-item tables can be execution evidence without being the business anchor that explains why work exists.
- Runtime observations remain investigation-scoped and never overwrite metadata confidence.
- Metadata-valid paths with no sampled records remain structurally valid for other records or contexts.
- No POST, PATCH, DELETE, upload, remediation or unverified query-generation tools are registered by Local MCP.
...
- Added `dvqr_resolve_navigation_property` to prevent guessed Dataverse navigation properties.
- Added `dvqr_explain_lookup` for standard and polymorphic lookup targets, value properties, runtime target annotations, and target-specific expands.
- Added layered structured OData execution errors with Dataverse error classification and actionable recovery guidance.
- Improved Windows PowerShell fallback handling so successful connectivity followed by a Dataverse HTTP error is distinguished from transport failure.
- Added synthetic relationship-path and structured-error tests without customer identifiers.
- Added `dvqr_generate_relationship_query` for metadata-verified direct, nested, and staged OData query templates.
- Added `dvqr_probe_relationship_path` for explicit bounded read-only probing of one source record across a verified path.
- Compact relationship-path responses now return the best five paths with concise ranking explanations and next actions.

# DV Quick Run v0.15.4 — Read-Only MCP Server Foundation

## Added

- Added deterministic relationship explainability with confidence labels, five-level ratings, scoring factors, business meaning, relationship categories, and educational tips.
- Added query-shape rationale with estimated cost and expected round trips for direct, nested, and staged relationship queries.
- Added relationship-purpose explanations for common Dataverse Customer, Primary Contact, Owner, Business Unit, Regarding, Activity, and hierarchy relationships.
- Refreshed the Welcome page and Hub around Talk to Dataverse relationship intelligence, verified query generation, bounded runtime probing, and explainability.
- Added explicit relationship-intent preservation for relationship path discovery, query generation, and runtime probing. Callers can provide a lookup logical name, navigation property, or relationship schema name such as `parentcustomerid`; DVQR will honour that relationship or return a bounded no-match result instead of silently selecting a different valid path.
- Added Minimal, Recommended, and Staged query variants to generated relationship-query output.
- Expanded path explanations to state when an explicitly requested relationship determined selection.
...
### v0.15.6 — Operational Workflow Discovery Pass

- Relationship discovery no longer terminates merely because enough direct paths were found.
- Bounded graph exploration now prioritises likely operational workflow hubs while preserving metadata-only truth.
- Direct relationships can no longer consume the complete candidate budget before bridged workflow families are considered.
- Partial relationship metadata failures, including a single relationship collection timeout, preserve usable metadata from the remaining collections.
- MCP relationship-path results now expose discovery coverage: direct and bridged paths found, tables inspected, operational hubs inspected, and whether bounded exploration completed.

### v0.15.6 implementation pass 6 — Operational Anchor Discovery

- Added `dvqr_discover_operational_anchors` for bounded, source-first workflow discovery without requiring a task target.
- Ranks likely business workflow anchors primarily from relationship centrality, bridge shape, source proximity and downstream work-item connectivity.
- Uses table names and descriptions only as supporting semantic evidence.
- Separates operational anchors, supporting anchors, downstream work items and platform infrastructure.
- Preserves metadata/runtime boundaries: anchor ranking does not claim matching records exist for the current investigation.
